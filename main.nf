...
//Channel containing all the SRA id of the samples of interest
//source: https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP017413&o=acc_s%3Aa

chr_list = Channel.of(1..22,'MT','X','Y') 
//Channel containing all the human chromosomes (including mitochondrial DNA)


///////////////////////////////////////////////////////////////////////////////////////////////////////
// SET UPS BEFORE MAPPING                                                                            //
///////////////////////////////////////////////////////////////////////////////////////////////////////

process getFASTQ {
//This process permits to collect the genomic sequence of the samples of interest 
//It creates two compressed fastqc files for each samples of interest (one corresponding 
//to the 5'3' sequencing and the other to the 3'5' sequencing)

    input:
    val SRAID from SRA

    output:
    //we triplicate the channel to use it in three different processes : 
    //mapFASTQ, fastqc and fastq_screen
...
//It creates a fasta file (by combining all the fasta files generated at the getChrSeq process) 
//that is then proceded by STAR which generated a lot of different files
...
//The BAM files indexation will speed processes that use them
    ...
    //publish mapping results with index useful for IGV visualization
...
//It counts the number of reads aligned on each gene of the human genome
    ...
    //as the indexedBAM channel contains both bam and the corresponding index, filter to 
    //only pass bam file to featureCounts
...
process countExon {
//This process permits to obtain, for each sample of interest, genes level of expression in 
...
//It counts the number of reads aligned on each exon 
    ...
    //as the indexedBAM channel contains both bam and the corresponding index, filter to only 
    //pass bam file to featureCounts
...
/* Not necessary as we count with countFeature and not DEXSeq python scripts
...
//DESEQ2 allows to determine if there is a significant difference in gene expression between the 
//wild and the mutate condition
...
//This process permits to perfom statistic analysis of the samples of interest
//DEXseq allows to determine if there is a significant difference in gene expression in case of 
//alternative splicing between the wild and the mutate condition
...
process fastqc {
//Fastqc allows to evaluate the quality of the reads made by RNA-sequencing
...
//Gets bowtie2 genome index used by fastq_screen
    output:
...
//Fastq_screen is used to compare the reads to a set of sequence databases in order to detect 
//potential sample contaminations. Update configuration to match
//path before running fastq_screen
...
process multi_qc{
//MultiQC summarises results from the quality control modules (fastQC and FastQ Screen), 
//the mapping module (STAR) and featureCounts
//Returns an html file containing a graphical representation of these results
